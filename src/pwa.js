...
const PWA_BUILD='adventure-home-v25';
const SHELL_CACHE='motocloud-shell-v25';
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=25',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('Moto Mission Adventure Home button installed.')})})}catch(error){console.error(error)}})}
