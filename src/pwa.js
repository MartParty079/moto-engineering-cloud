...
const PWA_BUILD='recorder-phase4-v44';
const ACTIVE_CACHES=['motocloud-app-v43','motocloud-runtime-v43','motocloud-images-v43'];
...
if('serviceWorker'in navigator){navigator.serviceWorker.addEventListener('controllerchange',()=>window.dispatchEvent(new CustomEvent('moto-app-cache-updated')));window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=44',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated'){void clearLegacyMotoCaches();console.info('Moto Mission Phase 4 v44 installed.');window.dispatchEvent(new CustomEvent('moto-app-cache-updated'))}})});}catch(error){console.error(error)}})}
