...
function addRideOverview(){
  ...
  card.innerHTML=`<div class="rowtop"><div><span class="eyebrow">UNIFIED RIDE</span><h3>Record, monitor, map, and review every ride</h3><p>One ride screen now handles GPS logging, configurable dashboards, live telemetry, Adventure mapping, and completed ride history.</p></div><span class="badge">LIVE</span></div><div class="rideOverviewGrid"><div class="rideOverviewMetric"><span>Status</span><strong>Ready</strong></div><div class="rideOverviewMetric"><span>Logger</span><strong>GPS + IMU</strong></div><div class="rideOverviewMetric"><span>History</span><strong>${ridesText.trim()}</strong></div></div><div class="actions"><button id="openRideCenterOverview" class="primary">Open Ride</button><button id="openRideLogOverview" class="secondary">View Ride Log</button></div>`;
