...
const chromium = engines[process.env.E2E_BROWSER || 'chromium'];
...
try {
  ...
  const login = async nextOwner => {
    ...
    await page.reload(); await page.locator('.simpleNav').waitFor({state:'attached'});
  ...
  await page.locator('#openRide').click();
  assert.equal(await page.locator('#rideRecovery').isVisible(), true);
  await page.locator('#rideClose').click();
