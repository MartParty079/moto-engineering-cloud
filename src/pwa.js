...
const PWA_BUILD='map-space-contrast-v18';
const SHELL_CACHE='motocloud-shell-v18';
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=18',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('Moto Mission map space and high contrast update installed.')})})}catch(error){console.error(error)}})}
