...
const PWA_BUILD='garage-center-phase2-v11';

async function clearLegacyMotoCaches(){
  if(!('caches' in window))return;
  try{
    const keys=await caches.keys();
    await Promise.all(keys.filter(key=>key.startsWith('motocloud-shell-')&&key!=='motocloud-shell-v11').map(key=>caches.delete(key)));
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=11',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('MotoCloud Garage Center phase two installed.')})})}catch(error){console.error(error)}})}
