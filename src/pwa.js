...
const PWA_BUILD='adventure-popup-exit-v22';
const SHELL_CACHE='motocloud-shell-v22';
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=22',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('Moto Mission Adventure route popup and exit update installed.')})})}catch(error){console.error(error)}})}
