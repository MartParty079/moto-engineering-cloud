...
GPX is available from main navigation and Map → Tools → GPX. Import GPX tracks, routes and waypoints (5 MB / 50,000 points maximum), choose a segment, inspect distance/elevation ascent, rename, reverse, display and export. Make route creates a separate GPX route with the same ordered coordinates and retains the source track. Separate segments never join across gaps. This is line-following/map display and GPX conversion, not road-snapped or turn-by-turn navigation, offline map packs, or a complete onX replacement. Files remain in memory across map close/reopen; refresh or sign-out clears them. Export is required for durable storage. XML parsing rejects document types/entities and invalid coordinates; names use text nodes/escaped XML. Large elevation gain values can reflect noisy source data.

Fullscreen: installation now requests `fullscreen` with a standalone fallback, unrestricted orientation, and iOS `black-translucent` so the OS does not reserve the former opaque bar. Map removes root padding/scrolling and fills the viewport. A Fullscreen button is available when supported. Safari browser chrome is OS-controlled; use the Home Screen installation for the closest fullscreen experience. Relaunch after an online reload; existing installations may retain older display metadata. Validate portrait/landscape, keyboard opening, GPX import/export, map layers and status-area readability on-device. Revert this commit to restore prior display behavior; HTML/worker revalidation and API exclusions are unchanged.
