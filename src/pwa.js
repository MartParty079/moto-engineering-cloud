...
const PWA_BUILD='speed-limit-status-v20';
const SHELL_CACHE='motocloud-shell-v20';
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=20',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('Moto Mission speed and limit display update installed.')})})}catch(error){console.error(error)}})}
