...
const PWA_BUILD='security-phase3-v12';

async function clearLegacyMotoCaches(){
  if(!('caches' in window))return;
  try{
    const keys=await caches.keys();
    await Promise.all(keys.filter(key=>key.startsWith('motocloud-shell-')&&key!=='motocloud-shell-v12').map(key=>caches.delete(key)));
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=12',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('MotoCloud security hardening and phase three installed.')})})}catch(error){console.error(error)}})}
function installGuide(){document.querySelector('#iosInstallOverlay')?.remove();const overlay=document.createElement('div');overlay.id='iosInstallOverlay';overlay.innerHTML=`<section class="iosInstallCard"><button class="iosInstallClose" aria-label="Close">×</button><img src="/app-icon.svg" alt="MotoCloud icon"><div><small>INSTALL ON IPHONE</small><h2>Add MotoCloud to your Home Screen</h2><ol><li>Tap the <b>Share</b> button in Safari.</li><li>Scroll down and tap <b>Add to Home Screen</b>.</li><li>Tap <b>Add</b>. MotoCloud will launch like a normal app.</li></ol><p>Your sign-in remains available and the app shell can open when connectivity is weak.</p></div></section>`;document.body.appendChild(overlay);overlay.querySelector('.iosInstallClose').onclick=()=>overlay.remove();overlay.onclick=e=>{if(e.target===overlay)overlay.remove()}}
