...
async function consumeUsage(provider, authorization) {
  ...
  try {
    ...
    return { allowed: false, status: 503, reason: error?.name === 'AbortError' ? 'Usage counter timed out' : 'Usage counter unavailable' };
  }
}
...
async function google(lat, lon, prevLat, prevLon, heading) {
  ...
  return { status: 'road', source: 'Google Roads', road: 'Google matched road', limit, confidence: 'High', type: 'Road', lanes: '—', diagnostic: 'Google matched the current GPS trail.' };
}
...
export default async function handler(req, res) {
  ...
  for (const provider of order) {
    ...
    if (!keyPresent) { attempts.push(`${provider}: key not configured`); continue; }
    ...
    if (!usage.allowed) {
      ...
      if (requested !== 'auto') {
        return res.status(usage.status || 503).json({
          error: usage.reason,
          provider,
          usage: { requestCount: usage.request_count ?? 0, monthlyCap: usage.monthly_cap ?? 0, remaining: usage.remaining ?? 0 }
        });
      }
      continue;
    }
    ...
    try {
      ...
      return res.status(200).json({
        ...
        usage: { provider, requestCount: usage.request_count, monthlyCap: usage.monthly_cap, remaining: usage.remaining },
        attempts
      });
    } catch (error) {
