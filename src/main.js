...
function auth(){app.innerHTML=`<main class="authPage"><section class="authStory" aria-label="About Marty Moto Party"><a class="authWordmark" href="/">M<span>Marty Moto Party</span></a><div class="authStoryCopy"><span class="eyebrow">YOUR MOTORCYCLE WORKSPACE</span><h1>Less admin.<br>More adventure.</h1><p>Your bikes, rides, and service history.<br>All in one place, ready when you are.</p><div class="authRoute" aria-hidden="true"><svg viewBox="0 0 520 180" fill="none"><path d="M-10 150C80 150 40 35 150 65S250 195 325 100 410 20 530 45" stroke="currentColor" stroke-width="2"/><path d="M-10 168C80 168 40 53 150 83S250 213 325 118 410 38 530 63" stroke="currentColor" stroke-opacity=".15" stroke-width="1"/><circle cx="150" cy="65" r="7" fill="currentColor"/><circle cx="325" cy="100" r="7" fill="currentColor"/></svg><span>RIDE. MAINTAIN. EXPLORE.</span></div></div><p class="authFootnote">A little more organized. A lot more ready.</p></section><section class="authPanel"><div class="authFormWrap"><span class="eyebrow">WELCOME TO YOUR GARAGE</span><h2>Make room for<br>your next ride.</h2><p class="authIntro">Sign in to your garage. New accounts require an invitation from an administrator.</p><form id="auth" class="stack"><label for="authEmail">Email address</label><input id="authEmail" name="email" type="email" placeholder="you@example.com" autocomplete="email" required><label for="authPassword">Password</label><input id="authPassword" name="password" type="password" placeholder="Enter your password" autocomplete="current-password" required><button class="primary" value="signin">Sign in</button></form><p id="msg" class="muted" role="status" aria-live="polite"></p><p class="authHint">Your motorcycles, maintenance, and ride history.</p></div></section></main>`;$('#auth').onsubmit=async e=>{
 e.preventDefault();
 const form=e.currentTarget,message=$('#msg'),fields=new FormData(form),signup=e.submitter?.value==='signup';
 const buttons=[...form.querySelectorAll('button')];
 buttons.forEach(button=>button.disabled=true);form.setAttribute('aria-busy','true');
 message.textContent=signup?'Creating your account…':'Signing in…';
 try{
  const credentials={email:fields.get('email'),password:fields.get('password')};
  const result=signup?await supabase.auth.signUp(credentials):await supabase.auth.signInWithPassword(credentials);
  if(message.isConnected)message.textContent=result.error?.message||(signup&&!result.data?.session?'Check your email for an account confirmation link, then return to sign in.':'Signed in. Opening your workspace…');
 }catch{if(message.isConnected)message.textContent='Unable to connect. Check your connection and try again.'}
 finally{if(form.isConnected){buttons.forEach(button=>button.disabled=false);form.removeAttribute('aria-busy')}}
}}
function shell(){
 const groups=[
  ['Workspace',[['dashboard','⌂','Overview'],['garageMode','⚡','Garage Mode'],['ai','✦','AI Assistant']]],
  ['Build',[['roadmap','✓','Work Packages'],['engineering','⌁','Engineering'],['pcb','▦','PCB Designer'],['firmware','⌘','Firmware']]],
  ['Operations',[['garage','◇','Motorcycles'],['parts','⬡','Parts & BOM'],['maintenance','◷','Maintenance'],['rides','↗','Ride Log']]],
  ['Knowledge',[['notes','≡','Notebook'],['media','□','Project Files']]]
 ];
 app.innerHTML=`<header class="top"><div class="brand"><button id="menu" class="icon menuButton" aria-label="Open navigation" aria-controls="nav" aria-expanded="false">☰</button><div class="brandMark"><span>M</span></div><div class="brandCopy"><h1>Moto Engineering</h1><p>Mission control</p></div></div><div class="topSearch"><span>⌕</span><input id="globalSearch" placeholder="Search your workspace" aria-label="Search your workspace"><kbd>⌘ K</kbd></div><div class="topActions"><span class="systemStatus"><i></i> Cloud workspace</span><button id="logout" class="userButton" title="Sign out"><span>${esc((session.user.email||'M').slice(0,1).toUpperCase())}</span><b>${esc((session.user.email||'User').split('@')[0])}</b></button></div></header><div class="layout"><nav id="nav" class="nav" aria-label="Main navigation"><button id="navClose" type="button">Close menu ×</button><div class="navIntro"><span>WORKSPACE</span><strong>Your workspace</strong><small>Build · Maintain · Explore</small></div>${groups.map(([label,items])=>`<div class="navGroup"><div class="navLabel">${label}</div>${items.map(([x,icon,l])=>`<button data-v="${x}" class="${view===x?'active':''}"><span class="navIcon">${icon}</span><span>${l}</span>${x==='ai'&&state.ai_change_proposals?.filter(p=>p.status==='pending').length?`<em>${state.ai_change_proposals.filter(p=>p.status==='pending').length}</em>`:''}</button>`).join('')}</div>`).join('')}<div class="navFooter"><span class="navFooterIcon">ME</span><div><b>Moto Mission</b><small>Build · Prove · Ride</small></div></div></nav><main id="main"></main></div><div id="searchResults" class="searchResults hidden"></div><div id="modal" class="modal hidden"></div><div id="toast" class="toast"></div>`;
 $('#menu').onclick=()=>{const open=$('#nav').classList.toggle('open');$('#menu').setAttribute('aria-expanded',String(open))};$('#navClose').onclick=()=>{$('#nav').classList.remove('open');$('#menu').setAttribute('aria-expanded','false')};$('#logout').onclick=()=>supabase.auth.signOut();$$('[data-v]').forEach(b=>b.onclick=()=>{view=b.dataset.v;shell();render()});$('#globalSearch').oninput=globalSearch;
...
function render(){
 ...
 if(view==='dashboard'){let done=state.tasks.filter(x=>x.status==='Complete').length,total=state.tasks.length,budget=state.parts.reduce((s,x)=>s+(x.qty||0)*(x.unit_cost||0),0),locked=state.tasks.filter(x=>!taskUnlocked(x)).length,proofReady=state.tasks.filter(x=>x.status!=='Complete'&&gatePass(x)).length;m.innerHTML=`<div class="hero card"><div><span class="eyebrow">YOUR WORKSPACE</span><h2>A little progress. Every day.</h2><p>Keep your garage in order and your next project moving.</p></div><details class="workspaceTools"><summary>Workspace tools</summary><div class="actions"><button class="secondary" data-order>Apply recommended order</button><button class="secondary" data-seed>Refresh workbook</button></div></details></div><div class="metrics">${metric('Complete',`${done}/${total}`)}${metric('Locked packages',locked)}${metric('Ready for approval',proofReady)}${metric('Planned budget',money(budget))}</div><div class="two"><div class="card"><h3>Next unlocked work</h3>${state.tasks.filter(x=>x.status!=='Complete'&&taskUnlocked(x)).slice(0,8).map(x=>`<div class="item" style="margin-bottom:8px"><b>${esc(x.title)}</b><div class="sub">${esc(x.work_type||'General')} · ${esc(x.stage)}</div><div class="progress" style="margin-top:8px"><i style="width:${taskCompletion(x)}%"></i></div></div>`).join('')||'<div class="empty">No unlocked work.</div>'}</div><div class="card"><h3>Proof enforcement</h3><p>Software requires source code. Physical work requires photos. CAD work requires design files. Calibration and testing require result files. Completion is blocked until all required proof and dependencies pass.</p></div></div>`}
 ...
 bind();window.dispatchEvent(new CustomEvent('moto-view-rendered',{detail:{view}}))
}
