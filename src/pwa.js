...
const PWA_BUILD='recorder-road-context-v45';
const ACTIVE_CACHES=['motocloud-app-v45','motocloud-runtime-v45','motocloud-images-v45'];
...
if('serviceWorker'in navigator){navigator.serviceWorker.addEventListener('controllerchange',()=>window.dispatchEvent(new CustomEvent('moto-app-cache-updated')));window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=45',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated'){void clearLegacyMotoCaches();console.info('Moto Mission road context v45 installed.');window.dispatchEvent(new CustomEvent('moto-app-cache-updated'))}})});}catch(error){console.error(error)}})}
