...
const PWA_BUILD='durable-rider-redesign-v47';
const ACTIVE_CACHES=['motocloud-app-v47','motocloud-runtime-v47','motocloud-images-v47'];
...
if('serviceWorker'in navigator){navigator.serviceWorker.addEventListener('controllerchange',()=>window.dispatchEvent(new CustomEvent('moto-app-cache-updated')));window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=47',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated'){void clearLegacyMotoCaches();console.info('Moto Mission rider refresh v47 installed.');window.dispatchEvent(new CustomEvent('moto-app-cache-updated'))}})});}catch(error){console.error(error)}})}
...
function addInstallButton(){
  ...
  const host=document.querySelector('.topActions')||document.querySelector('.authFormWrap');
  if(!host)return;
  let button=document.querySelector('#installMotoCloud');
  if(!button){button=document.createElement('button');button.id='installMotoCloud';button.type='button';button.innerHTML='<span>⇩</span><b>Install App</b>';button.setAttribute('aria-label','Install app');button.onclick=installGuide}
