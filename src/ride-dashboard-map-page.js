...
async function open() {
  ...
  overlay.innerHTML = `<main class="mapPanel"><header><div><p>LIVE MAP</p><h1>Map</h1></div><button id="mapClose" aria-label="Close">×</button></header><section class="mapToolbar"><label>Base map<select id="mapLayer">${Object.entries(layers).map(([id, layer]) => `<option value="${id}">${layer.label}</option>`).join('')}</select></label><button id="mapCenter">Center</button><button id="mapSettings">Settings</button></section><section class="mapCanvas" id="simpleMap"><div class="mapLoading">Loading map…</div></section><section class="mapReadout"><article><small>SPEED</small><strong id="mapSpeed">--</strong><span>MPH</span></article><article><small>SPEED LIMIT</small><strong id="mapLimit">--</strong><span id="mapLimitSource">UNKNOWN</span></article><article><small>GPS</small><strong id="mapGps">--</strong><span>ACCURACY</span></article></section><dialog id="mapSettingsDialog"><form method="dialog"><div class="dialogHead"><h2>Map settings</h2><button value="cancel">×</button></div><label class="switchRow">Follow location<input id="mapFollow" type="checkbox" checked></label><label class="switchRow">Keep screen awake<input id="mapWake" type="checkbox"></label><button class="primary" value="default">Done</button></form></dialog></main>`;
