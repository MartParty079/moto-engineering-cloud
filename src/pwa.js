...
const PWA_BUILD='map-ride-gauges-v49';
const ACTIVE_CACHES=['motocloud-app-v49','motocloud-runtime-v49','motocloud-images-v49'];
...
if('serviceWorker'in navigator){navigator.serviceWorker.addEventListener('controllerchange',()=>window.dispatchEvent(new CustomEvent('moto-app-cache-updated')));window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=49',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated'){void clearLegacyMotoCaches();console.info('Moto Mission rider refresh v49 installed.');window.dispatchEvent(new CustomEvent('moto-app-cache-updated'))}})});}catch(error){console.error(error)}})}
