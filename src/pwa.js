...
const PWA_BUILD='adaptive-speed-lean-v17';
const SHELL_CACHE='motocloud-shell-v17';
...
if('serviceWorker' in navigator){window.addEventListener('load',async()=>{try{const registration=await navigator.serviceWorker.register('/sw.js?v=17',{updateViaCache:'none'});await registration.update();registration.addEventListener('updatefound',()=>{const worker=registration.installing;if(!worker)return;worker.addEventListener('statechange',()=>{if(worker.state==='activated')console.info('Moto Mission adaptive speed and lean fixes installed.')})})}catch(error){console.error(error)}})}
