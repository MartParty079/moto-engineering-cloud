...
function countWheelies(rows){
 ...
 for(const r of rows){
  const pitch=finite(r.pitch_deg),speed=(finite(r.speed_mps)??0)*2.236936,now=Date.parse(r.recorded_at)||0;
  const raised=Number.isFinite(pitch)&&Math.abs(pitch)>=20&&speed>=5;
...
function leanStats(rows){
 const points=rows.map(row=>({row,value:finite(row.lean_deg)})).filter(point=>Number.isFinite(point.value));
 return {points,max:points.length?Math.max(...points.map(point=>Math.abs(point.value))):null}
}
function maxAccel(rows,ride){
 const points=rows.map(row=>finite(row.accel_g)).filter(Number.isFinite),stored=finite(ride?.max_accel_g);
 return Math.max(stored??0,...(points.length?points:[0]))
}

function openFullAnalytics(id){
 $('#rideDetailModal')?.remove();
 if(window.MotoRideAnalytics?.open){window.MotoRideAnalytics.open(id);return}
 window.dispatchEvent(new CustomEvent('moto-open-ride-analytics',{detail:{rideId:id}}));
}
...
async function showRideDetails(id){
 ...
 const rows=await loadSamples(id),wheelies=countWheelies(rows),lean=leanStats(rows),accel=maxAccel(rows,ride),old=$('#rideDetailModal');if(old)old.remove();
 const m=document.createElement('div');m.id='rideDetailModal';m.className='rideModal';
 m.innerHTML=`<section><header><div><small>RIDE LOG</small><h3>${esc(ride.bike_name)}</h3></div><button id="closeRideDetail">×</button></header><div class="rideDetailStats"><article><small>DATE</small><strong>${new Date(ride.started_at).toLocaleString()}</strong></article><article><small>DURATION</small><strong>${fmt(ride.duration_seconds)}</strong></article><article><small>DISTANCE</small><strong>${Number(ride.distance_miles||0).toFixed(2)} mi</strong></article><article><small>MAX SPEED</small><strong>${Number(ride.max_speed_mph||0).toFixed(1)} mph</strong></article><article><small>AVG SPEED</small><strong>${Number(ride.average_speed_mph||0).toFixed(1)} mph</strong></article><article><small>WHEELIES</small><strong>${wheelies}</strong></article><article><small>MAX LEAN</small><strong>${Number.isFinite(lean.max)?`${lean.max.toFixed(1)}°`:'--'}</strong></article><article><small>LEAN POINTS</small><strong>${lean.points.length}</strong></article><article><small>MAX ACCEL</small><strong>${accel.toFixed(2)} g</strong></article><article><small>ALL SAMPLES</small><strong>${rows.length}</strong></article></div><div class="rideLogActions"><button id="openRideAnalytics" class="rideStart">FULL RIDE ANALYTICS</button><button id="downloadRideCsv" class="rideStart">DOWNLOAD CSV</button></div><div class="rideSamplePreview"><h3>Recorded data</h3>${rows.length?`<p>${rows.length} GPS/IMU samples are stored, including ${lean.points.length} calibrated lean point${lean.points.length===1?'':'s'}.</p><div class="rideSampleTable"><div><b>First sample</b><span>${new Date(rows[0].recorded_at).toLocaleTimeString()}</span><span>${Number.isFinite(finite(rows[0].latitude))?finite(rows[0].latitude).toFixed(5):'—'}, ${Number.isFinite(finite(rows[0].longitude))?finite(rows[0].longitude).toFixed(5):'—'}</span></div><div><b>Last sample</b><span>${new Date(rows.at(-1).recorded_at).toLocaleTimeString()}</span><span>${Number.isFinite(finite(rows.at(-1).latitude))?finite(rows.at(-1).latitude).toFixed(5):'—'}, ${Number.isFinite(finite(rows.at(-1).longitude))?finite(rows.at(-1).longitude).toFixed(5):'—'}</span></div></div>`:'<div class="rideEmpty">This session has no stored sensor samples.</div>'}</div></section>`;
...
async function bikeSummaries(){
 ...
 for(const r of completed){
  ...
  if(!groups.has(key))groups.set(key,{name:r.bike_name||'Motorcycle',time:0,miles:0,wheelies:0,lean:null,rides:0,ids:[]});
  const g=groups.get(key);g.time+=Number(r.duration_seconds||0);g.miles+=Number(r.distance_miles||0);g.rides++;g.ids.push(r.id)
 }
 await Promise.all([...groups.values()].map(async g=>{
  const metrics=await Promise.all(g.ids.map(async id=>{const rows=await loadSamples(id);return{wheelies:countWheelies(rows),lean:leanStats(rows).max}}));
  g.wheelies=metrics.reduce((sum,item)=>sum+item.wheelies,0);
  const leanValues=metrics.map(item=>item.lean).filter(Number.isFinite);g.lean=leanValues.length?Math.max(...leanValues):null;
 }));
...
async function renderUnifiedRideLog(){
 ...
 const completed=sessions.filter(r=>r.status==='complete');
 const cards=await Promise.all(completed.map(async r=>{const rows=await loadSamples(r.id),wheelies=countWheelies(rows),lean=leanStats(rows);return`<article class="card rideLogCard" data-ride-session="${r.id}"><div><div class="eyebrow">${esc(r.status||'complete')}</div><h3>${esc(r.bike_name)}</h3><p>${new Date(r.started_at).toLocaleString()}</p></div><div class="rideLogSummary"><b>${Number(r.distance_miles||0).toFixed(2)} mi</b><span>${fmt(r.duration_seconds)}</span><span>${wheelies} wheelie${wheelies===1?'':'s'}</span><span>${Number.isFinite(lean.max)?`${lean.max.toFixed(1)}° lean`:'no calibrated lean'}</span><span>${lean.points.length} lean point${lean.points.length===1?'':'s'}</span><span>${Number(r.max_speed_mph||0).toFixed(1)} mph max</span></div><button class="mini">View log</button></article>`}));
