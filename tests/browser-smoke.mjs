...
const chromium = engines[engine];
const out = join('test-results', 'proof', engine);
...
try {
  ...
  await page.waitForFunction(()=>document.querySelector('#authMessage')?.textContent.includes('Local test rejection'));
  ...
  await page.reload();
  await page.locator('.simpleNav').waitFor();
  for (const width of [1280,390]) {
    await page.setViewportSize({width,height:844});
    await page.getByRole('heading',{name:'Garage',exact:true}).waitFor();
    await page.screenshot({path:join(out,'garage-'+width+'.png'),fullPage:true});
    await page.locator('#addBike').click();
    await page.getByRole('button',{name:'Cancel',exact:true}).click();
    assert.equal(await page.locator('dialog').count(),0);
    await page.locator('[data-view="service"]').click();
    await page.locator('#addService').click();
    await page.getByRole('button',{name:'Cancel',exact:true}).click();
    await page.locator('[data-view="history"]').click();
    await page.getByRole('heading',{name:'Ride history',exact:true}).waitFor();
    await page.locator('#openRide').click();
    await page.locator('#rideDashOverlay').waitFor();
    await page.screenshot({path:join(out,'ride-'+width+'.png'),fullPage:true});
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,'No horizontal overflow');
    await page.locator('#rideClose').click();
    await page.locator('[data-view="garage"]').click();
  }
  assert.equal(await page.locator('#openGpx').count(),1);
  assert.deepEqual(errors,[]);
  console.log(JSON.stringify({passed:['sign-in failure recovery','Garage','cancel forms','Service','Ride history','Ride at desktop and mobile widths'],screenshots:out}));
