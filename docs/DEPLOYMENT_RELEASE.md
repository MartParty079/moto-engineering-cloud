...
Keep additive schema during frontend rollback. Never drop completion markers/RPC while pending clients can retry. Export/reconcile journals before downgrading to an incompatible recorder. Verify canonical URL, main commit, worker v48, cache headers and bounded API JSON errors. Refresh existing tabs without clearing site data. Physical iPhone long-ride/background/hardware acceptance remains manual.
