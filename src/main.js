...
function shell(){
 const groups=[
  ['Command center',[['dashboard','⌂','Overview'],['garageMode','⚡','Garage Mode'],['ai','✦','AI Assistant']]],
 ...
 app.innerHTML=`<header class="top"><div class="brand"><button id="menu" class="icon menuButton" aria-label="Open navigation">☰</button><div class="brandMark"><span>M</span></div><div class="brandCopy"><h1>Moto Engineering</h1><p>Mission control</p></div></div><div class="topSearch"><span>⌕</span><input id="globalSearch" placeholder="Search tasks, parts, notes…"><kbd>⌘ K</kbd></div><div class="topActions"><span class="systemStatus"><i></i> Cloud synced</span><button id="logout" class="userButton" title="Sign out"><span>${esc((session.user.email||'M').slice(0,1).toUpperCase())}</span><b>${esc((session.user.email||'User').split('@')[0])}</b></button></div></header><div class="layout"><nav id="nav" class="nav"><div class="navIntro"><span>WORKSPACE</span><strong>Universal Data System</strong><small>Engineering build hub</small></div>${groups.map(([label,items])=>`<div class="navGroup"><div class="navLabel">${label}</div>${items.map(([x,icon,l])=>`<button data-v="${x}" class="${view===x?'active':''}"><span class="navIcon">${icon}</span><span>${l}</span>${x==='ai'&&state.ai_change_proposals?.filter(p=>p.status==='pending').length?`<em>${state.ai_change_proposals.filter(p=>p.status==='pending').length}</em>`:''}</button>`).join('')}</div>`).join('')}<div class="navFooter"><span class="navFooterIcon">ME</span><div><b>Moto Mission</b><small>Build · Prove · Ride</small></div></div></nav><main id="main"></main></div><div id="searchResults" class="searchResults hidden"></div><div id="modal" class="modal hidden"></div><div id="toast" class="toast"></div>`;
 $('#menu').onclick=()=>$('#nav').classList.toggle('open');$('#logout').onclick=()=>supabase.auth.signOut();$$('[data-v]').forEach(b=>b.onclick=()=>{view=b.dataset.v;shell();render()});$('#globalSearch').oninput=globalSearch;
 document.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();$('#globalSearch')?.focus()}},{once:true})
}
